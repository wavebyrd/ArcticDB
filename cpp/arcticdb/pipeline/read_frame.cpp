...
//     int16 view:   [ 10, 20, 30,  ?,  ?,  ?,  ?,  ?,  ?,  ?]
//     float32 view: [  ?,  ?,  ?,  ?, NaN, NaN, NaN, NaN, NaN]
...
void expand_and_promote_type(const util::BitMagic& bv, uint8_t* buf, DestType null_val = {}) {
    ...
    bm::bvector<>::size_type pos_sparse;
    bv.find_reverse(pos_sparse);
    for (auto pos_dense = static_cast<ssize_t>(bv.count()) - 1; pos_dense >= 0; --pos_dense) {
        ...
        bv.find_reverse(pos_sparse - 1, pos_sparse);
    }
}
